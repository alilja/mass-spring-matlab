...
k = randperm(size(left_edges,2)); 
selected_left = left_edges(k(1:num_segments));
selected_right = right_edges(k(1:num_segments));

% create the mesh
step = length/num_segments;
for(i = 1:num_segments)
    % create NODES and SPRINGS
    left_edge  = Node(i*3 - 2, start_pos + [selected_left(i).x selected_left(i).y],[0 0],10,0.5,1);
    spine      = Node(i*3 - 1, start_pos + [0 0],[0 0],10, 0.5);
    right_edge = Node(i*3, start_pos + [selected_right(i).x selected_right(i).y],[0 0],10, 0.5, 1);
    % a node is spine %
    k = 3;
    damp = 0.5;
...
% appears mirrored across a -1 45° line??

% render
imshow(edges);
hold on;
for(i = 1:system.num_nodes)
    circle(system.NODES(i).position(1), system.NODES(i).position(2), 10);
end
